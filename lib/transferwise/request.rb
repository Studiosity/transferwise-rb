module Transferwise
  class Request
    ...
    def self.handle_api_error(resp)
      ...
      error_message ||= error_obj['errors']&.map { |e| e['message'] }&.join(', ') || error_obj.to_s

      if Transferwise::STATUS_CLASS_MAPPING.include?(resp.code)
        raise "Transferwise::#{Transferwise::STATUS_CLASS_MAPPING[resp.code]}".constantize, error_params(
          error_message, resp, error_obj
        )
      else
        raise Transferwise::TransferwiseError, error_params(error_message, resp, error_obj)
      end
    end
    ...
    def self.handle_restclient_error(e, request_opts)
      ...
      message = case e
                when RestClient::RequestTimeout
                  "Could not connect to Transferwise (#{request_opts[:url]}). #{connection_message}"
                when RestClient::ServerBrokeConnection
                  "The connection to the server (#{request_opts[:url]}) broke before the " \
                  "request completed. #{connection_message}"
                else
                  'Unexpected error communicating with Transferwise. '
                end
